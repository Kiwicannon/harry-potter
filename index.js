...
//    const Enrollments = sequelize.define('courses', {
...
app.get('/api/students', (req, res) => {
    Students.findAll()
    .then( students => res.status(200).res.send(students))
...
app.get('/courses', (req, res) => {
    Courses.findAll()
    .then( courses => res.status(200).res.send( courses ))
    .catch(err => res.status(400).json({ error: err }))
})

app.put('/courses/:is', (req, res) => {
    const { coure_name } = req.body
    Courses.update({
        course_name
    }, {
    where: {
